...
const CounterReducer = () => {
  // useReduser의 첫 번째 파라미터에서는 리듀서 함수,
  // 두 번째 파라미터에는 해당 리듀서의 기본값
  // useReduser Hook을 사용하면 state 값과 dispatch 함수를 받아온다.
  // state : 현재 상태
  // dispatch : 액션을 발생시키는 함수
  // dispatch(action) 과 같은 형태로, 함수 안에 파라미터로 액션 값을 넣어주면 리듀서함수가 호출되는 구조다.
  // useReduser의 큰 장점은 컴포넌트 업데이트 로직을 컴포넌트 바깥으로 빼낼 수 있다.

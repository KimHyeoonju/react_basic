...
const Counter = () => {
  ...
  // [value] 에 [] 로 아무것도 없다면 처음 렌더링에서만 실행된다.
...
- 디스패치 : 주문하기
- 리듀서 : 바리스타, 주문 받은걸 만들고 가공해서 손님에게 내 준다.
...
- 실습예제 : 더블 클릭 방지 기능 구현, 사용자가 버튼을 빠르게 여러 번 클릭하는 경우 예상치 못하 여러 번의 액션을 방지
